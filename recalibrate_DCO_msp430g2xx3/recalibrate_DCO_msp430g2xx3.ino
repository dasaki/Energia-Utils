...
// 
// Write the checksum and adaptation to Energia
// by David Sanz Kirbis (oct 2015)
...
void loop()
{
  ...
  *Flash_ptrA = 0x00;                       // Dummy write to erase Flash seg A
  ...
  //
  // Write the checksum
  // added by David Sanz Kirbis (oct 2015)
  // based on info found here: http://www.simplyembedded.org/tutorials/msp430-configuration/
  //
  // The checksum is verified by XOR’ing the data as 16-bit words and then adding the checksum.
  // If the result is zero, the data is valid, any other value means the data is corrupted.
  //
  size_t len = 62 / 2;
  ...
  crc = -crc;
  Flash_ptrA = (char *)0x10c0; // The first 2 bytes of the TLV area (address 0x10c0) 
  *Flash_ptrA++ = *(crc_ptr++);
  *Flash_ptrA = *(crc_ptr);
  // end of checksum writing
